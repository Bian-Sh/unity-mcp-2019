...
def platform_join(platform: str, *parts: str) -> str:
    """Join path parts using the separators of the target Unity platform."""
    sep = "\\" if platform.startswith("win") else "/"
    cleaned = [str(part).strip("/\\").replace("/", sep).replace("\\", sep) for part in parts if str(part)]
    if not cleaned:
        return ""
    first = str(parts[0])
    result = first.rstrip("/\\").replace("/", sep).replace("\\", sep)
...
def hub_roots(platform: str | None = None, environ: dict[str, str] | None = None) -> list[str]:
    ...
    if plat.startswith("win"):
        ...
        for var in ("ProgramFiles", "ProgramFiles(x86)"):
            base = env.get(var)
            if base:
                roots.append(platform_join(plat, base, "Unity", "Hub", "Editor"))
        if not roots:
            roots.append(r"C:\Program Files\Unity\Hub\Editor")
        return roots
    # linux + everything else
    return [platform_join(plat, home, "Unity", "Hub", "Editor")]
...
def read_secondary_install_path(platform: str | None = None,
    ...
    home = env.get("HOME") or env.get("USERPROFILE") or str(Path.home())
    if plat == "darwin":
        cfg = platform_join(plat, home, "Library", "Application Support", "UnityHub", "secondaryInstallPath.json")
    elif plat.startswith("win"):
        appdata = env.get("APPDATA") or platform_join(plat, home, "AppData", "Roaming")
        cfg = platform_join(plat, appdata, "UnityHub", "secondaryInstallPath.json")
    else:
        xdg = env.get("XDG_CONFIG_HOME") or platform_join(plat, home, ".config")
        cfg = platform_join(plat, xdg, "UnityHub", "secondaryInstallPath.json")
...
def candidate_editor_paths(version: str,
    ...
    for root in hub_roots(plat, env):
        out.append(platform_join(plat, root, version, relpath))

    sec = read_secondary_install_path(plat, env, read_text)
    if sec:
        out.append(platform_join(plat, sec, version, relpath))
    ...
    for root in roots:
        ...
        for name in entries:
            ...
            if (pv[0], pv[1]) != (target[0], target[1]):
                continue
            binary = platform_join(plat, root, name, relpath)
