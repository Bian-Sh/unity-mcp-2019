...
def _fake_fs(present_paths: set[str], dirs: dict[str, list[str]] | None = None):
    ...
    for path in present_paths:
        expanded_paths.update(aliases(path))

    expanded_dirs: dict[str, list[str]] = {}
    for path, entries in (dirs or {}).items():
        for alias in aliases(path):
            expanded_dirs[alias] = entries

    def exists(p: str) -> bool:
        return p in expanded_paths

    def is_exec(p: str) -> bool:
        return p in expanded_paths

    def list_dir(d: str) -> list[str]:
        return list(expanded_dirs.get(d, []))
...
class TestDiscoverEditorPerOS:
    ...
    def test_windows_resolves_hub_layout(self):
        ...
        binary = lh.platform_join("win32", r"C:\Program Files", "Unity", "Hub", "Editor", version, "Editor", "Unity.exe")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            platform="win32",
    ...
    def test_linux_resolves_hub_layout(self):
        version = "6000.0.75f1"
        env = {"HOME": "/home/dev"}
        binary = lh.platform_join("linux", "/home/dev", "Unity", "Hub", "Editor", version, "Editor", "Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            platform="linux",
...
class TestDiscoverEditorMonkeypatchedPlatform:
    ...
    def test_default_platform_linux(self, monkeypatch):
        ...
        env = {"HOME": "/home/ci"}
        binary = lh.platform_join("linux", "/home/ci", "Unity", "Hub", "Editor", version, "Editor", "Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
        spec = resolve_editor_binary(
            version,
            environ=env,
...
class TestSecondaryInstallPath:
    ...
    def test_secondary_root_used_for_candidate(self):
        ...
        binary = lh.platform_join("darwin", secondary, version, "Unity.app/Contents/MacOS/Unity")
        exists, is_exec, list_dir = _fake_fs({binary})
